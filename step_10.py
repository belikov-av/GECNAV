...
def step10(input_dir: str = 'data', output_folder_path: str = 'data'):
    ...
    input_path = os.path.join(input_dir, 'EBPlusPlusAdjustPANCAN_IlluminaHiSeq_RNASeqV2-v2.geneExp_primary_whitelisted.tsv')

    if not os.path.isdir(output_folder_path):
        os.makedirs(output_folder_path)

    output_path = os.path.join(output_folder_path,
                               'EBPlusPlusAdjustPANCAN_IlluminaHiSeq_RNASeqV2-v2.geneExp_primary_whitelisted_median.tsv')

    if not os.path.isfile(output_path):

        print('We will read the file in a memory efficient way, line by line')
        print('There are 20531 strings in the file, so 20531 iterations required')
        ...
        with open(input_path, 'r') as input_file, open(output_path, 'w') as output_file:
            ...
            for line in tqdm(input_file):
                ...
                line = line.replace('\n', '')
                numbers = np.array([float(float_number) for float_number in line.split('\t')[1:]])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='This script takes ' +
                                                 ...
                                                 'file,' + '\n' + 'and and does cutoffs' + '\n' + \
                                                 'If output folder does not exist, script will create it.')
    parser.add_argument('-i', '--input_dir', type=str, help='full path to folder', default='data')
