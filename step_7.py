...
def step7(input_dir: str = 'data', output_folder_path: str = 'data'):
    ...
    input_path = os.path.join(input_dir, 'EBPlusPlusAdjustPANCAN_IlluminaHiSeq_RNASeqV2-v2.geneExp.tsv')
    ...
    if not os.path.isfile(output_file_path):
        ...
        all_samples = set(header[1:])
        print('There were {} samples'.format(len(all_samples)))

        new_columns = [header[i] for i in range(1)]

        selected_samples = set()
        for barcode in header[1:]:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='This script takes ' +
                                                 'EBPlusPlusAdjustPANCAN_IlluminaHiSeq_RNASeqV2-v2.geneExp.tsv file,' + '\n' +
